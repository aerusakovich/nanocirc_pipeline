process FILTER_CONFIDENT_DISCOVERY {
    ...
    // The output tsv and bed12 use the same names as their own inputs.
    // This is on purpose, it makes the "replace in place" publishDir trick
    // work. Nextflow's default stage-in is a symlink to the upstream
    // task's own work dir. Writing to a same-named output would then
    // follow that link and change the upstream task's file too, not just
    // this task's own copy. stageInMode 'copy' makes a real copy first, so
    // this task only ever changes its own file.
    stageInMode 'copy'
